...
void sieve() {
  isp.set();
  isp[0] = isp[1] = 0;
  primes.reserve(maxn / (log(maxn) - 2));
...
int main() {
  ...
  db(primes.size());
  while (cin >> a >> b >> k) {
    auto ub = upper_bound(primes.begin(), primes.end(), b);
    ...
    int i = lb - primes.begin();
    assert(primes[i + k - 1] <= b);
    int gap = primes[i + k - 1] - a + 1;
    for (; primes[i + k] <= b; ++i) {
      gap = max(gap, primes[i + k] - primes[i]);
    }
    gap = max(gap, b - primes[i] + 1);
