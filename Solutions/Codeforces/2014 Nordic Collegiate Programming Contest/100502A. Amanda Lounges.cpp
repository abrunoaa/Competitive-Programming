...
void dfs(int u) {
  ...
  for (auto [v, c] : g[u]) {
    debug(u, v, c);
    if (lounge[v] != -1 && lounge[u] + lounge[v] != c) {
      impossible();
    } else if (c == 2 && lounge[u] == 0) {
      impossible();
    } else {
      lounge[v] = c - lounge[u];
    }
  }
  for (auto [v, c] : g[u]) if (lounge[v] != -1 && !vis[v]) {
    dfs(v);
  }
}
...
tri cnt(int u) {
  vis[u] = 1;
  tri r(1, lounge[u], fix[u]);
  for (auto [v, c] : g[u]) if (!vis[v]) {
    tri s = cnt(v);
    r.a += s.a;
    r.b += s.b;
    r.c |= s.c;
