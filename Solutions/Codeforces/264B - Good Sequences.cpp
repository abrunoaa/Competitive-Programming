...
int n, prime_ans[(int)(maxn / (log(maxn) - 1)) + 1000];
bitset<maxn> isp;
vi primes;
...
void sieve(){
  ...
  assert(sizeof(prime_ans) > 4 * primes.size());
}
...
int main(){
  ...
  for(int i = 0, a; i < n; ++i){
    ...
    int max_good = 0;
    for(int x : f){
      max_good = max(max_good, prime_ans[x]);
    }
    ++max_good;
    ans = max(ans, max_good);
    for(int x : f){
      prime_ans[x] = max(prime_ans[x], max_good);
    }
  }
  cout << ans << endl;
