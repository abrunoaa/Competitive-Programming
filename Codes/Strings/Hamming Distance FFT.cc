/** Hamming Distance **/
#define maxn 200010
#define maxf (2 << (32 - clz(4*200000 - 1)))  // clz(max(2 * t.size(), 4 * p.size()) - 1)
#define ALFA 26                               // tamanho do alfabeto

char t[maxn], p[maxn];
int match[maxn];
cpx x[ALFA][maxf], y[ALFA][maxf];

// cada letra da entrada deve ser convertida em um inteiro
int reduce(char c){ return c - 'a'; }
